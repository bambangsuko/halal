...
class HomeMenuSubmenuController extends Controller
{
    ...
    public function create($menu_id)
    {
        ...
        return view('cp.home-menu.submenu.create', [
            'menu' => $menu,
    ...
    public function store(Request $request, $menu_id)
    {
        ...
        if ($menu->parent_id == 0) {
            return redirect(route('cp.home-menus.show', $menu->id))
                ->with('success', 'Submenu berhasil ditambahkan!');
        } else {
            return redirect(route('cp.home-menus.submenus.show', [$menu->parent_id, $menu->id]))
                ->with('success', 'Submenu berhasil ditambahkan!');
        }
    }
    ...
    public function show($menu_id, $submenu_id)
    {
        ...
        $submenu = Menu::where('id', $submenu_id)
            ...
            ->firstOrFail();
        return view('cp.home-menu.submenu.show', [
            'parent' => $menu,
            'menu' => $submenu,
    ...
    public function edit($menu_id, $submenu_id)
    {
        ...
        $submenu = Menu::where('id', $submenu_id)
            ->where('parent_id', $menu_id)
            ->firstOrFail();
        return view('cp.home-menu.submenu.edit', [
            'parent' => $menu,
            'menu' => $submenu,
            'templates' => Template::menu(),
            'breadcrumbs' => $this->breadcrumbRepository->generateByMenuId($menu->id)
    ...
    public function update($menu_id, $submenu_id, Request $request)
    {
        ...
        // } else {
        return redirect(route('cp.home-menus.submenus.show', [$submenu->parent_id, $submenu->id]))
            ->with('success', 'Submenu berhasil diubah!');
        // }
    }
    ...
    public function destroy($menu_id, $submenu_id)
    {
        ...
        $submenu = Menu::where('id', $submenu_id)
            ...
            ->firstOrFail();
        if ($menu->parent_id == 0) {
            $route = route('cp.home-menus.show', $menu->id);
        } else {
            $route = route('cp.home-menus.submenus.show', [$menu->parent_id, $menu->id]);
        }
